...
router.post("/addLocation", function (request, res, next) {
    ...
    connection.query(
      "INSERT INTO location(postal_code, city, state, latitude, longitude, isAvailable) VALUES ?",
      [insertValues],
...
router.put("/editLocation", function (request, res, next) {
    ...
    if (isAvailable != "") {
      ...
      connection.query(
        "UPDATE `location` SET `isAvailable` = ?, `lastUpdated` = CURRENT_TIMESTAMP WHERE `location_id` = ?;",
        [isAvailable, location_id],
        function (error, results, fields) {
          // If there is an issue with the query, output the error
...
router.delete(
  ...
  router.put("/editLocationBulk", verifyToken, function (request, res, next) {
    ...
    if (isAvailable != "") {
      ...
      connection.query(
        "UPDATE `location` SET `isAvailable` = ?, `lastUpdated` = CURRENT_TIMESTAMP WHERE `location_id` IN (?)",
        [isAvailable, location_ids],
        function (error, results, fields) {
          // If there is an issue with the query, output the error
          if (error) throw error;
          // If the account exists
