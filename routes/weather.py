...
def getlocations(allOrsingle, location_id):
    db = pymysql.connect(host="wsiwdb.ccciy990gxje.us-east-1.rds.amazonaws.com", user="wsiwdb",
                         passwd="wsiw1234", db="what_should_i_wear")
    cursor = db.cursor()
    if (allOrsingle == 'all'):
...
def getDataForLocations(weeklyOrhourly, allOrsingle):
    ...
    recordCount = 0
    connection = pymysql.connect(
        host="wsiwdb.ccciy990gxje.us-east-1.rds.amazonaws.com", user="wsiwdb", passwd="wsiw1234", db="what_should_i_wear")
    if (str(allOrsingle) == 'all' and str(weeklyOrhourly) == 'weekly'):
        ...
        for row in df_locations:
            ...
            cursor = connection.cursor()
            # if location isAvailable turned to No and still want to get 24 hrs data
            sqlCheck = "delete from weather where location_id = " + \
                str(row[4])+" and Date_Time <= "+"'" + \
                insertData1.Date_Time[23]+"'"+";"
            # print(sqlCheck)
